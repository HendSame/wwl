	...
	<table class="table">
		...
		<tr id="item_<?php  echo $item['id'];?>">
			...
			<td>
				...
				<button class="btn btn-sm btn-danger" onclick="deleteItem('<?php  echo $item['id'];?>')">移入未通过</button>
			</td>
...
<script type="text/javascript">
	...
	function deleteItem(id) {
		...
		if(!confirm('确认将该审核通过的记录移入审核未通过中？该出租信息将不在前台进行展示，操作无法回滚')) {
			return false;
		}

		$.ajax({
			type:'post',
			url:'<?php  echo $this->createWebUrl('HireItemDelete')?>',
			data:{rid:id},
			success:function(d) {
				if(d.res == '100') {
