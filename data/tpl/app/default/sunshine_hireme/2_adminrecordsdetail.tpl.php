...
<div class="weui_tabbar">
	 <a href="javascript:;" onclick="verifyRecordsStatus('allow')" class="weui_tabbar_item weui_bar_item_on">
        <div class="weui_tabbar_icon">
            <img src="<?php  echo $_W['siteroot'];?>addons/sunshine_hireme/common/img/icon_nav_msg.png" alt="">
        </div>
        <p class="weui_tabbar_label">通过</p>
    </a>
    <a href="javascript:;" onclick="verifyRecordsStatus('deny')" class="weui_tabbar_item weui_bar_item_on">
        <div class="weui_tabbar_icon">
            <img src="<?php  echo $_W['siteroot'];?>addons/sunshine_hireme/common/img/icon_nav_msg.png" alt="">
        </div>
        <p class="weui_tabbar_label" style="color:red">拒绝</p>
    </a>
</div>
...
<script>
  ...
  // 待审核 / 被拒绝 上架中 完成
  function verifyRecordsStatus(status) {
    if(status=='deny') {
        if(!confirm("确定拒绝通过？")) {
            return;
        }
    ...
    $.ajax({
        ...
        url:"<?php  echo $this->createMobileUrl('adminRecordsVerify')?>",
        success:function(d,s) {
            if(d.res == '100') {
                alert("操作成功");
                window.location.href="<?php  echo $this->createMobileUrl('adminrecordslistwait')?>";
            }else {
                alert(d.msg);
            }
        }
