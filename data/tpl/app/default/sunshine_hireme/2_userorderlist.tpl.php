...
<div class="container">
	<!-- <div class="weui_navbar">
        <div class="weui_navbar_item <?php  if($status=='wait') { ?>weui_bar_item_on<?php  } ?>" onclick="window.location.href='<?php  echo $this->createMobileUrl('UserOrderList',array('status'=>'wait'))?>'">
            待支付
        </div>
        <div class="weui_navbar_item <?php  if($status=='payed') { ?>weui_bar_item_on<?php  } ?>" onclick="window.location.href='<?php  echo $this->createMobileUrl('UserOrderList',array('status'=>'payed'))?>'"> 
            待回复
        </div>
        <div class="weui_navbar_item <?php  if($status=='refund') { ?>weui_bar_item_on<?php  } ?>" onclick="window.location.href='<?php  echo $this->createMobileUrl('UserOrderList',array('status'=>'refund'))?>'"> 
            已退款
        </div>
        <div class="weui_navbar_item <?php  if($status=='success') { ?>weui_bar_item_on<?php  } ?>" onclick="window.location.href='<?php  echo $this->createMobileUrl('UserOrderList',array('status'=>'success'))?>'"> 
            已完成
        </div>
    </div>
    ...
    <a href="<?php  echo $this->createMobileUrl('userorderdetail',array('orderid'=>$item['id']))?>" class="weui_media_box weui_media_appmsg">
...
function listload() {
	...
	$.ajax({
		...
		url : "<?php  echo $this->createMobileUrl('userorderlistajax')?>",
		success :function(d) {
			if(d.res == '100') {
				var html = '';
				for (var i in d.list) {
